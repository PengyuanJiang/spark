...
/**
 * A command that uploads data into a Hive table
 *
 * The syntax of this command is:
 * {{{
 *   UPLOAD DATA INPATH 'filepath' [OVERWRITE] INTO TABLE tablename
 * }}}
 *
 */
case class UploadDataCommand(
   ...
   override def run(sparkSession: SparkSession): Seq[Row] ={
     ...
     if (targetTable.tableType == CatalogTableType.VIEW) {
       throw new AnalysisException(s"Target table in UPLOAD DATA cannot be a view: $tableIdentwithDB")
     }
     if (DDLUtils.isDatasourceTable(targetTable)) {
       throw new AnalysisException(
